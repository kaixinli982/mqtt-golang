...
func (c *client) Subscribe(topic string, qos byte, callback MessageHandler) Token {
	...
	token.subErrors = make(map[string]error) // 初始化错误信息map
	DEBUG.Println(CLI, "enter Subscribe")
	if !c.IsConnected() {
		err := fmt.Errorf("subscribe failed: client not connected to broker")
		token.setError(err)
		token.subErrors[topic] = err
		return token
	}
	if !c.IsConnectionOpen() {
		...
		case !c.options.ResumeSubs:
			...
			err := fmt.Errorf("subscribe failed: client not connected and ResumeSubs is false, subscription will be discarded")
			token.setError(err)
			token.subErrors[topic] = err
			return token
		case c.options.CleanSession && c.status.ConnectionStatus() == reconnecting:
			...
			err := fmt.Errorf("subscribe failed: client is reconnecting and CleanSession is true, subscription will be discarded")
			token.setError(err)
			token.subErrors[topic] = err
			return token
		}
	...
	if err := validateTopicAndQos(topic, qos); err != nil {
		token.setError(err)
		token.subErrors[topic] = fmt.Errorf("subscribe failed: invalid topic or QoS - %v", err)
		return token
	}
	...
	if sub.MessageID == 0 {
		mID := c.getID(token)
		if mID == 0 {
			err := fmt.Errorf("subscribe failed: no message IDs available for topic %s", topic)
			token.setError(err)
			token.subErrors[topic] = err
			return token
		}
	...
	default:
		...
		case <-time.After(subscribeWaitTimeout):
			err := fmt.Errorf("subscribe failed: timeout after %v while sending subscribe message for topic %s", subscribeWaitTimeout, topic)
			token.setError(err)
			token.subErrors[topic] = err
		}
	}
...
func (c *client) SubscribeMultiple(filters map[string]byte, callback MessageHandler) Token {
	...
	if !c.IsConnected() {
		...
		for topic := range filters {
			token.subErrors[topic] = ErrNotConnected
		}
		return token
	}
	if !c.IsConnectionOpen() {
		...
		case !c.options.ResumeSubs:
			// if not connected and resumesubs not set this sub will be thrown away
			err := fmt.Errorf("not currently connected and ResumeSubs not set")
			token.setError(err)
			for topic := range filters {
				token.subErrors[topic] = err
			}
		...
		case c.options.CleanSession && c.status.ConnectionStatus() == reconnecting:
			// if reconnecting and cleanSession is true this sub will be thrown away
			err := fmt.Errorf("reconnecting state and cleansession is true")
			token.setError(err)
			for topic := range filters {
				token.subErrors[topic] = err
			}
	...
	if sub.MessageID == 0 {
		mID := c.getID(token)
		if mID == 0 {
			err := fmt.Errorf("no message IDs available")
			token.setError(err)
			for topic := range filters {
				token.subErrors[topic] = err
			}
	...
	default:
		...
		case c.oboundP <- &PacketAndToken{p: sub, t: token}:
		case <-time.After(subscribeWaitTimeout):
			err := errors.New("subscribe was broken by timeout")
			token.setError(err)
			for topic := range filters {
				token.subErrors[topic] = err
			}
